...
import { Activity, Users, BarChart2, Terminal, Zap, Command, TrendingUp, Shield, Loader2 } from 'lucide-react';
...
const LC: Record<string, string> = { INFO: '#7A7F8A', WARN: '#D4A017', SUCCESS: '#E2E4E9', ALERT: '#D95B16' };
...
export const Dashboard: React.FC = () => {
    const [metrics, setMetrics] = useState<MetricState>({ totalLeads: 0, activeAgents: 0, loading: true });
    ...
    useEffect(() => {
        ...
        const fetchPipeline = async () => {
            ...
            const stages: PipelineStage[] = Object.entries(counts).map(([stage, count]) => ({
                stage, count, pct: Math.round((count / total) * 100),
            }));
            setPipeline(stages.slice(0, 5));
        };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 52 }}>
            ...
            <PhoenixCard variant="deep" dotted className="relative overflow-hidden" style={{ minHeight: 330, padding: '52px 64px 0' }}>
                <motion.div className="relative pb-14" style={{ zIndex: 2, maxWidth: 560 }}
                    ...
                    <p style={{ fontFamily: BD, fontSize: 14, lineHeight: 1.65, color: '#9CA0A8', marginBottom: 28, maxWidth: 420 }}>
                        ...
                        <span style={{ color: '#FF7A29', fontWeight: 600 }}>
                            {metrics.loading ? '…' : `${metrics.totalLeads} active leads`}
                        </span>
                        {' '}and{' '}
                        <span style={{ color: '#FFA057', fontWeight: 600 }}>
                            {metrics.loading ? '…' : `${metrics.activeAgents} agents`}
                        </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Total Leads */}
                <PhoenixCard variant="charcoal" className="p-8 flex flex-col justify-between" style={{ minHeight: 180 }}>
                    <div className="flex justify-between items-start mb-6">
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                            <Activity size={15} style={{ color: '#7A7F8A' }} />
                        </div>
                        <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#FF7A29', boxShadow: '0 0 7px rgba(255,122,41,0.7)' }} />
                    </div>
                    <h3 style={{ fontFamily: H, fontSize: 'clamp(28px,3vw,38px)', fontWeight: 800, color: '#FFF', letterSpacing: '-0.5px', lineHeight: 1, marginBottom: 10 }}>
                        {metrics.loading ? metricSpinner : metrics.totalLeads}
                    </h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ padding: '2px 8px', borderRadius: 6, fontFamily: BD, fontSize: 10, fontWeight: 700, background: 'rgba(255,122,41,0.1)', color: '#FF7A29', border: '1px solid rgba(255,122,41,0.22)' }}>LIVE</span>
                        <span style={{ fontFamily: BD, fontSize: 11, color: '#5A5F69' }}>Total Leads</span>
                    </div>
                </PhoenixCard>

                {/* Active Agents */}
                <PhoenixCard variant="charcoal" glow className="p-8 flex flex-col justify-between" style={{ minHeight: 180 }}>
                    <div className="flex justify-between items-start mb-6">
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                            <Users size={15} style={{ color: '#7A7F8A' }} />
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ fontFamily: MN, fontSize: 9, color: '#FF7A29', letterSpacing: '0.2em' }}>LIVE</span>
                            <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#FF7A29', boxShadow: '0 0 7px rgba(255,122,41,0.7)' }} />
                        </div>
                    </div>
                    <h3 style={{ fontFamily: H, fontSize: 'clamp(28px,3vw,38px)', fontWeight: 800, color: '#FFF', letterSpacing: '-0.5px', lineHeight: 1, marginBottom: 10 }}>
                        {metrics.loading ? metricSpinner : `${metrics.activeAgents} / 6`}
                    </h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ padding: '2px 8px', borderRadius: 6, fontFamily: BD, fontSize: 10, fontWeight: 700, background: 'rgba(255,122,41,0.1)', color: '#FF7A29', border: '1px solid rgba(255,122,41,0.22)' }}>+2 Deployed</span>
                        <span style={{ fontFamily: BD, fontSize: 11, color: '#5A5F69' }}>Active Agents</span>
                    </div>
                </PhoenixCard>
                ...
                <PhoenixCard variant="charcoal" className="p-8 flex flex-col justify-between" style={{ minHeight: 180 }}>
                    <div className="flex justify-between items-start mb-6">
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                            <BarChart2 size={15} style={{ color: '#7A7F8A' }} />
                        </div>
                        <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#2A2D32' }} />
                    </div>
                    <h3 style={{ fontFamily: H, fontSize: 'clamp(28px,3vw,38px)', fontWeight: 800, color: '#FFF', letterSpacing: '-0.5px', lineHeight: 1, marginBottom: 10 }}>
                        {metrics.loading ? metricSpinner : metrics.totalLeads}
                    </h3>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ padding: '2px 8px', borderRadius: 6, fontFamily: BD, fontSize: 10, fontWeight: 700, background: 'rgba(255,255,255,0.04)', color: '#7A7F8A', border: '1px solid rgba(255,255,255,0.06)' }}>Stable</span>
                        <span style={{ fontFamily: BD, fontSize: 11, color: '#5A5F69' }}>Lead Pipeline</span>
                    </div>
                </PhoenixCard>
            </div>
                    ...
                    <div style={{ padding: '8px 28px 16px' }}>
                        {logsLoading ? (
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '20px 8px' }}>
                                <Loader2 size={14} style={{ color: '#FF7A29', animation: 'spin 1s linear infinite' }} />
                                <span style={{ fontFamily: MN, fontSize: 10, color: '#3A3F4A' }}>Fetching logs…</span>
                            </div>
                        ) : logs.length === 0 ? (
                            <div style={{ padding: '20px 8px' }}>
                                <span style={{ fontFamily: MN, fontSize: 10, color: '#2A2F3A', letterSpacing: '0.12em' }}>NO LOGS YET</span>
                            </div>
                        ) : (
                            logs.map((log, i) => (
                                <React.Fragment key={log.id}>
                                    <motion.div style={{ display: 'grid', gridTemplateColumns: '5.5rem 4.5rem 1fr', gap: 16, padding: '10px 8px', borderRadius: 8 }}
                                        ...
                                        <span style={{ fontFamily: MN, fontSize: 10, color: '#3A3F4A' }}>{formatTime(log.created_at)}</span>
                                        <span style={{ fontFamily: MN, fontSize: 9, fontWeight: 700, color: LC[log.level] ?? '#7A7F8A', letterSpacing: '0.12em' }}>{log.level}</span>
                                        <span style={{ fontFamily: BD, fontSize: 12, color: '#9CA0A8' }}>{log.message}</span>
                                    </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <PhoenixCard variant="deep" dotted className="p-7">
                    ...
                    {pipeline.length === 0
                        ? [{ stage: 'New', pct: 25, count: 0 }, { stage: 'Contacted', pct: 40, count: 0 }, { stage: 'Qualifying', pct: 17, count: 0 }, { stage: 'Proposal', pct: 10, count: 0 }, { stage: 'Won', pct: 8, count: 0 }].map(({ stage, pct, count }, i) => (
                        ...
                        : pipeline.map(({ stage, count, pct }, i) => (
                            <div key={stage} style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                                <span style={{ fontFamily: BD, fontSize: 11, color: '#5A5F69', minWidth: 80, textTransform: 'capitalize' }}>{stage}</span>
                                <div style={{ flex: 1, height: 4, borderRadius: 99, background: 'rgba(255,255,255,0.07)', overflow: 'hidden' }}>
                                    <motion.div style={{ height: '100%', borderRadius: 99, background: 'linear-gradient(90deg,#D95B16,#FF7A29,#FFA057)' }} initial={{ width: 0 }} animate={{ width: `${Math.max(pct * 3.5, 4)}%` }} transition={{ duration: 1, ease: 'easeOut', delay: 0.15 * i }} />
                                </div>
                                <span style={{ fontFamily: BD, fontSize: 12, fontWeight: 700, color: '#D0D3DA', minWidth: 20, textAlign: 'right' }}>{count}</span>
                            </div>
                        ))
