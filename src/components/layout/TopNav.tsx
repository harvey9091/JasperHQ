...
import { Bell, Grid, Zap, MessageCircle } from 'lucide-react';
...
export const TopNav: React.FC = () => {
    const { isLuffyChatOpen, setLuffyChatOpen } = useAgentStatusStore();
    return (
        <header
        ...
        >
            ...
            <div className="flex items-center gap-3 shrink-0">
                ...
                <button
                    onClick={() => setLuffyChatOpen(!isLuffyChatOpen)}
                    className="w-9 h-9 rounded-full flex items-center justify-center text-[#7A7F8A] hover:text-[#FF7A29] transition-all"
                    style={{
                        background: isLuffyChatOpen ? 'rgba(255,122,41,0.1)' : 'rgba(255,255,255,0.04)',
                        border: `1px solid ${isLuffyChatOpen ? 'rgba(255,122,41,0.3)' : 'rgba(255,255,255,0.06)'}`
                    }}
